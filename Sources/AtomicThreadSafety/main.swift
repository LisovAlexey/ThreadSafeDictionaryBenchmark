...
func benchmarkDictionaryCreation<Dictionary: DictProtocolMutable>(_ keys: [Dictionary.Key],
                                                           _ values: [Dictionary.Value],
                                                           _ dict: inout Dictionary) {
    for (key, value) in zip(keys, values) {
        dict.mutate {$0[key] = value}
    }
...
//benchmark.addSimple(
//    title: "ThreadSafeUserDict - Class Atomic NSLock",
//    input: ([Int], [Int]).self
//) { keys, values in
//    var dict = ThreadSafeUserDictClassAtomicNSLock(from: [Int: Int]())
//
//    benchmarkDictionaryCreation(keys, values, &dict)
//}
...
benchmark.addSimple(
    title: "ThreadSafeUserDict - Class Atomic Clean with mutation",
    input: ([Int], [Int]).self
) { keys, values in
    var dict = ThreadSafeUserDictClassAtomicMutatingClean(from: [Int: Int]())
...
protocol DictProtocolMutable {
    ...
    func mutate(_ mutation: (inout Dictionary<Key, Value>) -> Void)
...
@propertyWrapper
class ClassAtomicMutatingClean<Value> {
    private var value: Value
    
    var projectedValue: ClassAtomicMutatingClean<Value> {
        return self
    }
...
final class ThreadSafeUserDictClassAtomicNSLock<Key: Hashable & Equatable, Value>: DictProtocol {
    
    var _dict: ClassAtomicNSLock<Dictionary<Key, Value>>
    ...
    init(from dict: Dictionary<Key, Value>) {
        self._dict = ClassAtomicNSLock(wrappedValue: dict)
    }
...
final class ThreadSafeUserDictClassAtomicMutatingClean<Key: Hashable & Equatable, Value>: DictProtocolMutable {
    subscript(key: Key) -> Value? {
        _dict.wrappedValue[key]
    }
    
    func mutate(_ mutation: (inout Dictionary<Key, Value>) -> Void) {
        _dict.mutate(mutation)
    }
    
    var _dict: ClassAtomicMutatingClean<Dictionary<Key, Value>>
    ...
    init(from dict: Dictionary<Key, Value>) {
        self._dict = ClassAtomicMutatingClean(wrappedValue: dict)
    }
